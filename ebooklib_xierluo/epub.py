...
NAV_XML = six.b('''<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" 
xmlns:epub="http://www.idpf.org/2007/ops" 
xmlns:xml='http://www.w3.org/XML/1998/namespace'/>''')

CHAPTER_XML = six.b('''<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" 
xmlns:epub="http://www.idpf.org/2007/ops" 
xmlns:xml='http://www.w3.org/XML/1998/namespace' 
...
class EpubBook(object):
    ...
    def set_identifier(self, uid):
        ...
        self.set_unique_metadata('DC', 'identifier', self.uid, {'id': '轻国地址'})
    ...
    def add_item(self, item):
        ...
        if item.media_type == '':
            ...
            if has_guessed:
                ...
                if item.get_name().split('.')[1] == 'avif':
                    item.media_type = 'image/avif'
                else:
...
class EpubWriter(object):
    ...
    def __init__(self, name, book, options=None):
        ...
        if book.EPUB_VERSION:
            self.options['version'] = book.EPUB_VERSION
    ...
    def _get_nav(self, item):
        ...
        content_title = etree.SubElement(nav, 'p', {'class': "contents"})
        content_title.text = "CONTENTS"

        def _create_section(itm, items):
            div_ = etree.SubElement(itm, 'div')
            for item in items:
                if isinstance(item, tuple) or isinstance(item, list):
                    p_item = etree.SubElement(div_, 'p', {'class': "mulu"})
                    if isinstance(item[0], EpubHtml):
                        a = etree.SubElement(p_item, 'a', {'href': os.path.relpath(item[0].file_name, nav_dir_name)})
                    elif isinstance(item[0], Section) and item[0].href != '':
                        a = etree.SubElement(p_item, 'a', {'href': os.path.relpath(item[0].href, nav_dir_name)})
                    elif isinstance(item[0], Link):
                        a = etree.SubElement(p_item, 'a', {'href': os.path.relpath(item[0].href, nav_dir_name)})
                    else:
                        a = etree.SubElement(p_item, 'span')
                    a.text = item[0].title

                    _create_section(p_item, item[1])

                elif isinstance(item, Link):
                    p_item = etree.SubElement(div_, 'p', {'class': "mulu"})
                    a = etree.SubElement(p_item, 'a', {'href': os.path.relpath(item.href, nav_dir_name)})
                    a.text = item.title
                elif isinstance(item, EpubHtml):
                    p_item = etree.SubElement(div_, 'p', {'class': "mulu"})
                    a = etree.SubElement(p_item, 'a', {'href': os.path.relpath(item.file_name, nav_dir_name)})
                    a.text = item.title
